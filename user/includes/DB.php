...
function order($order_key){
	global $db;
	$query = "SELECT orders.* , category.name as category, service.name as service,deadline.name as deadline,subject.name as subject, academic_level.name as academic_level, user.table as user_table
		FROM orders
...
function search_order($data){
	...
	// $date2 = $data['date2'];
		...
		global $db;
	$sql ="SELECT orders.id,orders.order_key,menus.id,menus.items,orders.qty,menus.price,orders.created_at FROM orders LEFT JOIN menus ON menus.id=orders.item_id WHERE orders.created_at BETWEEN " . "'" . $data['date1'] . "'" . " AND " . "'" . $data['date2'] . "'";
...
function get_record_id($table,$id,$fields=null){
	global $db;
...
function insert_user($data){
	...
	$username = $data['username']; 	
 	 ...
 	 $query = "INSERT INTO users('table','username') VALUES('$table','$username')";
...
function get_user($table,$table,$fields=null){

	global $db;
	
	 $query = "SELECT id FROM $table WHERE table = '$table'";
	$query = $db->query($query);
...
function remove_where($table,$id,$fields=null){
	...
	$query = "DELETE from $table where order_key = '$id'";
	$query = $db->query($query);
